...
def unified_handler(path):
    ...
    if request.method == 'GET':
        if 'debug' in request.path:
            return debug()
        return jsonify({"message": "API is running. POST to chat to interact.", "path": path, "request_path": request.path}), 200

    # Assume all POST requests are intended for the chat endpoint
